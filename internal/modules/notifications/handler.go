...
func (h *Handler) List(c *fiber.Ctx) error {
	...
	if err != nil {
		return response.Failure(c, appErrors.InvalidNotificationData)
	}
...
func (h *Handler) GetByID(c *fiber.Ctx) error {
	...
	if err != nil {
		if errors.Is(err, appErrors.NotificationNotFound) {
			return response.Failure(c, appErrors.NotificationNotFound)
		}
		if typed, ok := err.(*appErrors.Error); ok {
			return response.Failure(c, typed)
		}
	...
	return response.Success(c, notification, nil)
}
